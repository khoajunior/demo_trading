...
const { EMIT_CLOSE_PENDING_ORDER_LIST, EMIT_GET_ORDER_LIST } = require('../../constants/constants')
const { v4: uuid } = require('uuid')

//item: id_list,user_id
module.exports = (item) => new Promise(async(resolve, reject) => {
    try {
        ...
        //send input to EMIT_CLOSE_PENDING_ORDER_LIST
        const input = item || {
            request_id: uuid(),
            id_list: [396, 397] //341->347,363->370
        }
        socket_forex.emit(EMIT_CLOSE_PENDING_ORDER_LIST, input)
...
const listen_data = (socket, socket_active, type) => {
    ...
    socket.on(EMIT_CLOSE_PENDING_ORDER_LIST, data => {
