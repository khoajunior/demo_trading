...
const { EMIT_GET_ORDER_LIST, EMIT_CLOSE_ACTIVE_ORDER_LIST } = require('../../constants/constants')
const { v4: uuid } = require('uuid')

module.exports = (item = null) => new Promise(async(resolve, reject) => {
    console.log('Test auto close_active_order_list')
    try {
        ...
        //send input to EMIT_CLOSE_ACTIVE_ORDER_LIST
        const input = item || {
            request_id: uuid(),
            // id_list: [255, 2s56]
            id_list: null
        }
        socket_forex.emit(EMIT_CLOSE_ACTIVE_ORDER_LIST, input)
...
const listen_data = (socket, socket_active, type) => {
    ...
    socket.on(EMIT_CLOSE_ACTIVE_ORDER_LIST, data => {
