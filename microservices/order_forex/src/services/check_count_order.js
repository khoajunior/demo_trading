...
module.exports=(item)=>new Promise(async(resolve,reject)=>{
    try{
        ...
        const GET_COUNT=`query MyQuery($user_id: uuid!,$tournament_id:uuid!) {
            demo_history_forex_aggregate(where: {tournament_id: {_eq: $tournament_id}, user_id: {_eq: $user_id}, status: {_in: [2,3]}}) {
              aggregate {
                count
              }
            }
        ...
        const count = response.data.demo_history_forex_aggregate.aggregate.count
