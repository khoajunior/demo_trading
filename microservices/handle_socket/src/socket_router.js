...
const {
    ...
    HANDLE_SOCKET_CLIENT_QUEUE,
    EMIT_RELOAD_ORDER_ACTIVE,
    EMIT_RELOAD_ORDER_PENDING,
    EMIT_RELOAD_ORDER_PENDING_CANCEL
...
module.exports = () => {
    ...
    messageBroker.addConsumer(HANDLE_SOCKET_CLIENT_QUEUE, async  msg => {
        const content = msg.content.toString()
        const { key_list } = JSON.parse(content)

        const routing_key = msg.fields.routingKey
        // console.log(key_list)


        if (routing_key == "data.order.pending") {
            await handle_socket_update_order(router_chart, EMIT_RELOAD_ORDER_PENDING, key_list)
            return

        }

        if (routing_key == 'data.order.active') {
            await handle_socket_update_order(router_chart, EMIT_RELOAD_ORDER_ACTIVE, key_list)
            return
        }

        if (routing_key == 'data.order.pending-active') {
            await handle_socket_update_order(router_chart, EMIT_RELOAD_ORDER_PENDING, key_list)
            await handle_socket_update_order(router_chart, EMIT_RELOAD_ORDER_ACTIVE, key_list)
            return
        }

        if (routing_key == 'data.order.pending-active-cancel') {
            await handle_socket_update_order(router_chart, EMIT_RELOAD_ORDER_PENDING_CANCEL, key_list)
            return
        }
    }, { noAck: true })
...
const handle_socket_update_order = (router_chart, emit_name, key_list) => new Promise(async (resolve, reject) => {
    try {
        const unique_socket_list = await get_unique_socket_list()
        ...
        for (var i = 0; i < unique_socket_list.length; i++) {
            ...
            if (key_list.includes(key)) {
                router_chart.to(`demo_account/${key}`).emit(emit_name, {reload: new Date().toISOString()})
            }
        }
        return resolve(true)
    } catch (error) {
